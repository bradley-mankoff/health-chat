#!/usr/bin/env bash
set -euo pipefail
# Start llama-server with Qwen3-27B Q4_K_M GGUF.
# Usage: bash scripts/run_llama.sh [path/to/gguf]
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
MODEL="${1:-$ROOT/models/qwen3-27b-q4_k_m.gguf}"
if [[ ! -f "$MODEL" ]]; then
  echo "GGUF not found: $MODEL" >&2
  echo "Download Qwen3-27B Q4_K_M to that path — see docs/MODELS.md" >&2
  exit 1
fi
