...
echo "  2) Start LLM (authenticated, loopback only):  bash scripts/run_llama.sh"
echo "     (shares LLM_API_KEY with server.py via .llm_api_key; see docs/MODELS.md)"
echo "     or MLX on Mac (unauthenticated, testing only):  bash scripts/run_mlx.sh"
